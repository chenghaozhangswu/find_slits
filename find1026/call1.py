...
def componemt_dist(data,sample_para_w):
    ...
    for i in range(data.shape[0]):  # data.shape[0]2
        # print(i)
        a = data[i, :] 
        a[:][a<-50] = 0
        w = 5  # 采用频率为10的时候w=5 
        ...
        location,_ = find_peaks(a_var, height=0.017,distance=1)
...
def getlist(filepath):
    ...
    name.sort(key=lambda x: int(x[-2:]))
    return name
...
def show_component_distance(component_distance,sample_para_w,sample_para_h,img,threshold):
    ...
    for j in range(len(Search)):
        ...
        '''
        if Search['var_l'][j]<100 and Search['var_r'][j]<100 and 0.001*(Search['d'][j])*sample_para_w*5<10: # 上下方差小于20 且 最小距离小于thredhold才画出来
            choosed_lunkuo.append(j)
        '''
        #choosed_lunkuo.append(j)

        if Search['var_l'][j] < 100 and Search['var_r'][j] < 100:
            choosed_lunkuo.append(j)
    ...
    for i in range(len(choosed_lunkuo)-1):
        ...
        if choosed_lunkuo[i+1]-choosed_lunkuo[i]>1:
            ...
            print('choosed_lunkuo: ', choosed_lunkuo[i])
            real_distance=indx[j,3]*sample_para_w*5 # 降采样频率为 计算成真实的微米
            print('真实间距:',0.001*real_distance,'毫米     ', indx[j,3])


            if real_distance > 0:
                distbox = np.append(distbox, real_distance)  # 间隙保存到distbox排序
                numbox = np.append(numbox, i)
    ...
    if len(distbox)>0:
        ...
        num = np.argmin(distbox)
        i = int(numbox[num])
        j = choosed_lunkuo[i]
        img = cv2ImgAddText(img, str(np.min(distbox)/1000)+'毫米', int(indx[j, 1]), int(indx[j, 0]), (255, 255, 255), 20)
        pt1 = [int(indx[j, 1]), int(indx[j, 0])]
        pt2 = [int(indx[j, 2]), int(indx[j, 0])]
        # cv2.arrowedLine(img, pt1, pt2, (255, 0, 0), 5)
        cv2.line(img, pt1, pt2, (255, 255, 255), 5)
        cv2.imwrite("findcomponent.jpg", img)
    return img,Search,str(np.min(distbox))
...
# 是否用180度的数据对其
    ...
    sample_para_w=5
    sample_para_h=int(sample_para_w/xihua)
    ...
    path_height='D:\jianxi/20221111/1/'
    path_luminance='D:\jianxi/20221111/2/'
    test = 'test'
    ...
    if open_duiqi==1:
        ...
        img,Search_Component,cm=show_component_distance(component_distance,sample_para_w,sample_para_h,img,5000)
        print()
    ...
    def show_pins_distance(pins_distance1, img):
        ...
        while (j < len(Pins_distance1)):
            ...
            if 1:
                ...
                real_distance = Pins_distance1['distance'][j] * 20 * 5/1000  # 降采样频率为10 计算成真实的毫米
                # real_distance = round(real_distance, 3)
                # img = cv2ImgAddText(img, str(real_distance) + '毫米', int(Pins_distance1['left'][j]),
                #                     int(Pins_distance1['row'][j]) + 10, (255, 255, 0), 20)
                distbox = np.append(distbox, real_distance)
                numbox = np.append(numbox, j)
                # cv2.arrowedLine(img, pt1, pt2, (0, 255, 0), 1)
                j = j+1  # 5个轮廓之间只显示一个

        if len(distbox) > 0:
            pm = str(np.min(distbox))
            num = np.argmin(distbox)
            j = numbox[num]
            pt1 = (int(Pins_distance1['left'][j]), int(Pins_distance1['row'][j]))
            pt2 = (int(Pins_distance1['right'][j]), int(Pins_distance1['row'][j]))
            img = cv2ImgAddText(img, pm + '毫米', int(Pins_distance1['left'][j]),
                                int(Pins_distance1['row'][j]), (255, 255, 255), 20)
            cv2.line(img, pt1, pt2, (255, 255, 255), 1)
            cv2.imwrite("./datasets/resultImg/{}.jpg".format(test), img)
        else:
            pm = "未发现引脚"
        return Pins_distance1, pm

    # img = cv2.imread('color_rgb.jpg')

    # 保存点云数据
    data_new = pd.DataFrame(200*oh)
    data_new.to_csv(r"./datasets/csv/{}.csv".format('Name'), header=False, index=False)
    pins_distance1 = find_pins.pins_dist(oh)
    b,pm=show_pins_distance(pins_distance1, img)

    jx = float(cm)/1000
    print(jx)
    # p1 = multiprocessing.Process(target=find_pins.pins_dist, args=(oh,))
    # p2 = multiprocessing.Process(target=show_pins_distance, args=(pins_distance1, img,))
    # p1.start()
    # p2.start()
    return cm,pm
a,b = start()
